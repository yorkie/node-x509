...
void	get_pem_from_p12(const FunctionCallbackInfo<Value> &args)
{
  ...
  Local<Object> exports(extract_pem_from_p12(args[0]->ToString(), args[1]->ToString()));
  args.GetReturnValue().Set(exports);
}
...
Handle<Value> get_pem_from_p12(const Arguments &args) {
  ...
  Handle<Value> exports(extract_pem_from_p12(*path, *password));
...
Handle<Value> extract_pem_from_p12(char *data, char* password) {
  ...
  size_t length = BIO_get_mem_data(mem, &output);
  sk_X509_pop_free(ca, X509_free);
  X509_free(cert);
  EVP_PKEY_free(pkey);
  pemCert = String::New(output, length);
  BIO_free_all(mem);
  return (scope.Close(pemCert));
}
